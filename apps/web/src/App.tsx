...
function Label({ text }: { text: string }) {
    return (
        <div style={{ color: '#374151', fontWeight: 600, fontSize: 11, marginBottom: 4 }}>
            {text}
        </div>
    );
}
        ...
        <div>
            ...
            {events.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                    <div
                        style={{ fontSize: 12, color: '#374151', fontWeight: 600, marginBottom: 6 }}
                    >
                        Timeline
                    </div>
                    <div
                    ...
                    >
                        {events.map((ev, i) => (
                            ...
                            >
                                ...
                                {ev.type === 'error' && (
                                    <span style={{ color: '#dc2626' }}>✗ {ev.message}</span>
                                )}
                            </div>
                        ))}
                    </div>
...
function AppConfigTab() {
    ...
    useEffect(() => {
        ...
        apiFetch<{ appKey: string; name: string; config: Record<string, unknown> }>(
            `/apps/${selectedKey}/config`,
        )
            .then((d) => {
                const c = d.config;
                ...
                setForm({
                    name: d.name,
                    selectedRepos: ((c['repos'] as Array<Record<string, string>>) ?? []).map(
                        (r) => ({
                            name: r['name'] ?? '',
                            url: r['url'] ?? '',
                    ...
                    dbSources: ((c['dbSources'] as Array<Record<string, string>>) ?? []).map(
                        (s) => ({
                            key: s['key'] ?? '',
                            type: s['type'] ?? 'sql',
                            connectionString: '',
    ...
    const buildBody = () => {
        ...
        try {
            expectedConfigParsed = JSON.parse(form.expectedConfig || '{}');
        } catch {}
        return {
            name: form.name,
            ...(form.selectedRepos.length > 0
                ? {
                      repos: form.selectedRepos.map((r) => ({
                          name: r.name,
                          url: r.url,
            ...
            ...(form.dbSources.length > 0
                ? {
                      dbSources: form.dbSources.map((s) => ({
                          key: s.key,
                          type: s.type,
                          connectionString: s.connectionString,
                          ...(s.mgmtUrl ? { mgmtUrl: s.mgmtUrl } : {}),
                      })),
                  }
                : {}),
        ...
        try {
            ...
            if (creating) {
                ...
                await apiFetch('/apps', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ appKey: newAppKey, ...body }),
                });
                setSelectedKey(newAppKey);
                setCreating(false);
                setNewAppKey('');
            } else {
                await apiFetch(`/apps/${selectedKey}/config`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(body),
                });
    ...
    const toggleRepo = (item: GitLabRepoItem) => {
        ...
        if (already) {
            setFormField(
                'selectedRepos',
                form.selectedRepos.filter((r) => r.gitlabProjectId !== id),
            );
        } else {
            setFormField('selectedRepos', [
                ...form.selectedRepos,
                {
                    name: item.name,
    ...
    return (
        <div>
            ...
            >
                <select
                    value={creating ? '__new__' : selectedKey}
                    onChange={(e) => {
            ...
            {(selectedKey || creating) && (
                <div>
                    ...
                    <div style={sectionStyle}>
                        ...
                        >
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormField('gitlabBaseUrl', e.target.value)}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormField('gitlabToken', e.target.value)}
                                    placeholder="glpat-..."
                                    style={{ ...inp, width: '100%', boxSizing: 'border-box' }}
                                />
                            </div>
                        ...
                        >
                            <div style={{ flex: 1 }}>
                                <Label text="Group ID" />
                                <input
                                    value={form.gitlabGroupId}
                                    onChange={(e) => setFormField('gitlabGroupId', e.target.value)}
                            ...
                            <button
                                ...
                                style={{ ...btn('#4b5563'), whiteSpace: 'nowrap' }}
                            >
                            ...
                            <div>
                                ...
                                >
                                    {fetchedRepos.map((item) => {
                                        const checked = form.selectedRepos.some(
                                            (r) => r.gitlabProjectId === String(item.id),
                                        );
                                        return (
                                            <label
                                                key={item.id}
                    ...
                    <ArraySection
                        ...
                        onAdd={() =>
                            setFormField('dbSources', [
                                ...form.dbSources,
                                { key: '', type: 'sql', connectionString: '', mgmtUrl: '' },
                            ])
                        }
                        onRemove={(i) =>
                            setFormField(
                                'dbSources',
                                form.dbSources.filter((_, j) => j !== i),
                            )
                        }
                        renderItem={(s, i) => (
                            <div
                            ...
                            >
                                <input
                                    value={s.key}
                                    onChange={(e) =>
                                        updateArr<DbEntry>('dbSources', i, { key: e.target.value })
                                    }
                                    placeholder="key (e.g. main-db)"
                                    style={inp}
                                />
                                <select
                                    value={s.type}
                                    onChange={(e) =>
                                        updateArr<DbEntry>('dbSources', i, { type: e.target.value })
                                    }
                                    style={inp}
                                >
                                    {['sql', 'mongo', 'redis', 'rabbitmq'].map((t) => (
                                        <option key={t}>{t}</option>
                                    ))}
                                </select>
                                <input
                                    type="password"
                                    value={s.connectionString}
                                    onChange={(e) =>
                                        updateArr<DbEntry>('dbSources', i, {
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        updateArr<DbEntry>('dbSources', i, {
                                            mgmtUrl: e.target.value,
                                        })
                                    }
                                    placeholder="mgmtUrl (rabbitmq)"
                                    style={inp}
                                />
                            </div>
                        )}
                    ...
                    <div style={sectionStyle}>
                                ...
                                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                    ...
                                    {learnMsg && (
                                        <span
                                            style={{
                                                fontSize: 11,
                                                color: learnMsg.startsWith('Error')
                                                    ? '#dc2626'
                                                    : '#166534',
                                            }}
                                        >
                        ...
                        >
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormField('appStoreUrl', e.target.value)}
                            ...
                            <div>
                                ...
                                <input
                                    value={form.homepage}
                                    onChange={(e) => setFormField('homepage', e.target.value)}
                        ...
                        <div style={{ marginBottom: 10 }}>
                            ...
                            <textarea
                                ...
                                onChange={(e) => setFormField('appDescription', e.target.value)}
                                rows={2}
                        ...
                        <div>
                            ...
                            <textarea
                                value={form.docUrls}
                                onChange={(e) => setFormField('docUrls', e.target.value)}
                                rows={3}
                                placeholder={
                    ...
                    <div style={sectionStyle}>
                        ...
                        <textarea
                            value={form.expectedConfig}
                            onChange={(e) => setFormField('expectedConfig', e.target.value)}
                            rows={4}
                            style={{
                                ...inp,
                                width: '100%',
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <button
                            ...
                            style={btn(saving ? '#9ca3af' : '#166534')}
                        >
    ...
    return (
        <div style={sectionStyle}>
            ...
            >
                ...
                <button
                    onClick={onAdd}
                    style={{ ...btn('#374151'), padding: '3px 10px', fontSize: 11 }}
                >
                    + Add
                </button>
            </div>
            {items.length === 0 && (
                <div style={{ color: '#9ca3af', fontSize: 11 }}>None configured</div>
            )}
            {items.map((item, i) => (
                <div
                    key={i}
    ...
    return (
        <div>
            ...
            >
                ...
                <button onClick={load} style={btn('#374151')}>
                    Refresh
                </button>
                    ...
                    {expanded === run.runId && (
                        ...
                        >
                            <div style={{ color: '#6b7280', marginBottom: 6 }}>
                                ...
                                <strong>Reported by:</strong> {run.reportedBy}
                            </div>
                            {run.output && (
                                <div
                                    style={{
                                ...
                                >
                                    {run.output.caseType && (
                                        <MiniField label="Case Type" value={run.output.caseType} />
                                    )}
                                    {run.output.confidence && (
                                        <MiniField
                                            label="Confidence"
                                            value={run.output.confidence}
                                        />
                                    )}
