...
router.post('/apps/:appKey/config/test', async (req, res) => {
  ...
  const results = [
    { surface: 'repos', key: 'repos', ok: resolved.repos.length > 0, message: `${resolved.repos.length} repo(s) configured` },
    { surface: 'database', key: 'dbSources', ok: resolved.dbSources.length > 0, message: `${resolved.dbSources.length} DB source(s) configured` },
    { surface: 'logs', key: 'logSources', ok: (resolved.logSources?.length ?? 0) > 0, message: resolved.logSources ? `${resolved.logSources.length} log source(s)` : 'not configured (optional)' },
    { surface: 'shopify', key: 'shopify', ok: Boolean(resolved.shopify?.adminToken), message: resolved.shopify ? 'Shopify configured' : 'not configured' },
  ];
  res.json({ results });
});
...
function maskSecrets(cfg: Record<string, unknown>): Record<string, unknown> {
  ...
  if (out['gitlab'] && typeof out['gitlab'] === 'object') mask(out['gitlab'] as Record<string, unknown>, ['token']);
  if (Array.isArray(out['dbSources'])) {
    (out['dbSources'] as Array<Record<string, unknown>>).forEach((s) => mask(s, ['connectionString', 'mgmtUrl']));
  }
  if (Array.isArray(out['logSources'])) {
    (out['logSources'] as Array<Record<string, unknown>>).forEach((s) => mask(s, ['endpoint', 'token']));
  }
  if (out['shopify'] && typeof out['shopify'] === 'object') mask(out['shopify'] as Record<string, unknown>, ['adminToken']);
