...
export async function investigateLogs(
    ...
    if (!appConfig?.logSources?.length) {
        return {
            ...
            found: false,
            data: null,
            reason: 'logSources not configured (optional)',
            provenance: 'logs:none',
        };
    }
    ...
    if (!source) {
        return {
            ...
            found: false,
            data: null,
            reason: `Log source "${sourceKey}" not found`,
            provenance: 'logs:sources',
        };
    }
    ...
    try {
        const lines = await queryLogs(source, {
            ...
            timeWindowMinutes: probe.target['timeWindowMinutes']
                ? Number(probe.target['timeWindowMinutes'])
                : 60,
        });
