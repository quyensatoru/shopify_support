...
export async function investigateConfig(
    ...
    if (!appConfig) {
        return {
            ...
            found: false,
            data: null,
            reason: 'App config not available',
            provenance: 'config:none',
        };
    }
    ...
    if (probe.action === 'diff_expected') {
        ...
        for (const [key, expectedVal] of Object.entries(expected)) {
            diffs.push({
                ...
                actual: '(runtime value — not checked at planning time)',
            });
        }
        return {
            ...base,
            status: 'done',
            found: diffs.length > 0,
            data: diffs,
            provenance: 'config:diff_expected',
        };
    }
    ...
    return {
        ...
        found: false,
        data: null,
        reason: `Unknown config action: ${probe.action}`,
        provenance: 'config',
    };
}
