...
export async function investigateBrowser(probe: Probe, request: RunRequest): Promise<ProbeResult> {
        ...
        if (probe.action === 'check_markers') {
            ...
            if (!marker)
                return {
                    ...
                    found: false,
                    data: null,
                    reason: 'No marker to check',
                    provenance: `browser:${url}`,
                };
            const found =
                page.html.includes(marker) || page.scripts.some((s) => s.includes(marker));
            return {
                ...base,
                status: 'done',
                found,
