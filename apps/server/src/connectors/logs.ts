...
async function queryLoki(source: LogSource, query: LogQuery): Promise<string[]> {
  ...
  if (!res.ok) return [];
  const data = await res.json() as { data?: { result?: Array<{ values?: Array<[string, string]> }> } };
  return (data.data?.result ?? []).flatMap((r) => (r.values ?? []).map(([, line]) => line)).slice(0, 50);
}
...
async function queryElastic(source: LogSource, query: LogQuery): Promise<string[]> {
  ...
  if (!res.ok) return [];
  const data = await res.json() as { hits?: { hits?: Array<{ _source?: { message?: string } }> } };
...
async function queryGeneric(source: LogSource, query: LogQuery): Promise<string[]> {
  ...
  return [`[logs:${source.type}] unsupported connector — no logs retrieved for keyword="${query.keyword ?? ''}"`];
}
