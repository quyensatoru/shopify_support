...
export class RedisAdapter implements DbAdapter {
  ...
  async checkExists(_target: string, key: string): Promise<{ exists: boolean; sample?: unknown }> {
  ...
  async keyInspect(pattern: string): Promise<unknown> {
    ...
    for (const key of keys.slice(0, 5)) {
      ...
      const value = type === 'string' ? await this.client.get(key) ?? undefined : undefined;
