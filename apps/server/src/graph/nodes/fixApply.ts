...
export async function fixApplyNode(state: SupportStateType) {
  ...
  if (!state.fixPlan || !state.appConfig) {
    return {
      ...
      timeline: [stepLog('fixApply', 'skipped', Date.now() - t0, 'Missing fixPlan or appConfig')],
    };
  }
  ...
  try {
    ...
    return {
      artifacts,
      timeline: [stepLog('fixApply', 'completed', Date.now() - t0, artifacts.mrUrl ? `MR: ${artifacts.mrUrl}` : 'Fix applied')],
    };
  } catch (err) {
    return {
